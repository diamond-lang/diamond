...
std::string get_lines(
    size_t start_line, size_t end_line, std::filesystem::path file_path
);
...
std::string underline_identifier(
           ...
           (location.line > 1
                ? std::to_string(location.line - 1) + "| " +
                      current_line(Location{
                          location.line - 1, location.column, location.file
                      }) +
                      "\n"
                : "") +
...
std::string errors::expecting_new_indentation_level(Location location) {
    return make_header("Expecting new indentation level\n\n") +
           std::to_string(location.line - 1) + "| " +
           current_line(location.line - 1, location.file) + "\n" +
           std::to_string(location.line) + "| " + current_line(location) +
           "\n" + underline_current_char(location);
}

std::string errors::undefined_variable(
    ast::IdentifierNode& identifier, std::filesystem::path file
) {
    return make_header("Undefined variable\n\n") +
           std::to_string(identifier.line) + "| " +
           current_line(identifier.line, file) + "\n" +
           underline_identifier(identifier, file);
}

std::string errors::reassigning_immutable_variable(
    ...
    return make_header("Trying to reassign immutable variable\n\n") +
           ...
           "| " + current_line(declaration.line, file);
}

std::string errors::undefined_function(
...
std::string errors::undefined_function(
    ast::CallNode& call, std::vector<ast::Type> args, std::filesystem::path file
) {
    auto& identifier = call.identifier->value;
    return make_header("Undefined function\n\n") + identifier + "(" +
           format_args(args) + ") is not defined.\n\n" +
...
static std::string list_functions(std::vector<ast::FunctionNode*> functions) {
    ...
    for (auto function : functions) {
        result += "    " + std::to_string(function->line) + "| " +
                  current_line(function->line, function->module_path) + "\n";
    }
    return result;
}
...
std::string errors::unhandled_return_value(
    ast::CallNode& call, std::filesystem::path file
) {
...
std::string get_lines(
    ...
    while (current <= end_line) {
        ...
        result += std::to_string(current) + "│ " +
                  current_line(current, file_path) + "\n";
        current++;
    }
    return result;
}
...
std::string current_line(Location location) {
    return current_line(location.line, location.file);
}
std::string current_line(size_t line, std::filesystem::path file_path) {
    if (file_path == "") return "";
