...
namespace codegen {
    ...
    struct Context {
        ...
        struct Binding {
            ...
            Binding() {}
            Binding(ast::Node *node, llvm::AllocaInst *pointer)
                : node(node), pointer(pointer) {}
            Binding(ast::Node *node, llvm::AllocaInst *pointer, bool is_mutable)
                : node(node), pointer(pointer), is_mutable(is_mutable) {}
            ~Binding() {}
        };
        ...
        struct Scopes {
            std::vector<std::unordered_map<std::string, Binding>>
                variable_scopes;
        ...
        CollectionAsArguments get_struct_type_as_argument(
        ...
        llvm::AllocaInst *copy_expression_to_memory(
        ...
        llvm::Value *codegen(ast::FunctionArgumentNode &node) {
            return nullptr;
        }
        ...
        std::vector<llvm::Value *> codegen_args(
            ast::FunctionNode *function,
            std::vector<ast::CallArgumentNode *> args
        );
        llvm::Value *codegen(ast::CallArgumentNode &node) { return nullptr; }
        llvm::Value *codegen_size_function(
            std::variant<llvm::Value *, llvm::AllocaInst *> pointer,
            ast::Type type
        );
        llvm::Value *codegen_print_struct_function(
            ast::Type arg_type, llvm::Value *arg_pointer
        );
        llvm::Value *codegen_call(
