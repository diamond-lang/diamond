...
ast::Type semantic::get_binding_type(semantic::Binding& binding) {
    switch (binding.type) {
        case semantic::VariableBinding:
            return ast::get_type(
                ((ast::DeclarationNode*)binding.value)->expression
            );
        case semantic::FunctionArgumentBinding:
            return ast::get_type(binding.value);
        case semantic::FunctionBinding:
            return ast::Type("function");
        case semantic::InterfaceBinding:
            return ast::Type("interface");
        case semantic::TypeBinding:
...
std::string semantic::get_binding_identifier(semantic::Binding& binding) {
    switch (binding.type) {
        case semantic::VariableBinding:
            return ((ast::DeclarationNode*)binding.value)->identifier->value;
        case semantic::FunctionArgumentBinding:
            return ((ast::IdentifierNode*)binding.value)->value;
        case semantic::FunctionBinding:
            return ((ast::FunctionNode*)binding.value)->identifier->value;
        case semantic::InterfaceBinding:
            return ((ast::InterfaceNode*)binding.value)->identifier->value;
        case semantic::TypeBinding:
            return ((ast::TypeNode*)binding.value)->identifier->value;
    }
    assert(false);
    return "";
}
...
semantic::Scope semantic::current_scope(Context& context) {
    ...
    return semantic::Scope{
        context.scopes
            .variables_scopes[context.scopes.variables_scopes.size() - 1],
        context.scopes.functions_and_types_scopes
            .scopes[context.scopes.functions_and_types_scopes.scopes.size() - 1]
    };
}

std::optional<semantic::Binding> semantic::get_binding(
    ...
    for (auto scope = context.scopes.functions_and_types_scopes.scopes.rbegin();
...
ast::Type semantic::new_type_variable(Context& context) {
    ast::Type new_type = ast::Type(
        ast::TypeVariable(context.type_inference.current_type_variable_number)
    );
...
void semantic::add_parameter_constraint(
...
void semantic::set_unified_type(
...
ast::Type semantic::new_final_type_variable(Context& context) {
    ...
    if (context.current_function.has_value()) {
        while (true) {
            ...
            for (auto& parameter :
                 context.current_function.value()->type_parameters) {
    ...
    ast::Type new_type = ast::Type(ast::FinalTypeVariable(
        as_letter(context.type_inference.current_type_variable_number)
    ));
    context.type_inference.current_type_variable_number++;
    return new_type;
...
std::vector<ast::Type> semantic::get_possible_types_for_argument(
...
std::vector<ast::Type> semantic::get_possible_types_for_return_type(
