...
Result<Ok, Error> semantic::analyze_block_or_expression(
    ...
    assert(
        std::holds_alternative<ast::BlockNode>(*node)
        || ast::is_expression(node)
    );
        ...
        while (j < type_constraints[i].size()) {
            ...
            for (size_t k = j + 1; k < type_constraints[i].size(); k++) {
                if (type_constraints[i].elements[j].is_nominal_type()
                    ...
                    if (type_constraints[i].elements[j].as_nominal_type().name
                        == type_constraints[i]
                               .elements[k]
                               .as_nominal_type()
                               .name) {
    ...
    for (auto type : context.type_inference.parameter_constraints) {
        ...
        semantic::set_unified_type(
            context,
            semantic::get_unified_type(context, type.first),
            unified_type
        );
    }
    ...
    if (context.current_function.has_value()
        && context.current_function.value()->state
               == ast::FunctionBeingAnalyzed) {
        ...
        for (size_t i = 0; i < context.current_function.value()->args.size();
            ...
            if (!context.current_function.value()->args[i]->type.is_concrete(
                ...
                add_type_parameters(
                    context,
                    *context.current_function.value(),
                    context.current_function.value()->args[i]->type,
                    unified_field_constraints,
                    unified_interface_constraints
                );
            }
        }
...
bool semantic::are_types_compatible(
    ...
    if (function_type.is_concrete()) {
        ...
        if (function_type.as_nominal_type().name
            ...
            for (size_t i = 0;
                 i < function_type.as_nominal_type().parameters.size();
                 i++) {
                if (!semantic::are_types_compatible(
                        function,
                        function_and_types_scopes,
...
bool semantic::are_types_compatible(
    ...
    std::vector<ast::Type> argument_types
) {
    ...
    if (function_type.is_final_type_variable()
        ...
        if (ast::get_type_parameter(type_parameters, function_type)
            ...
            ast::TypeParameter* type_parameter
                = ast::get_type_parameter(type_parameters, function_type)
                      .value();
            auto& field_constraints
                = type_parameter->type.as_final_type_variable()
                      .field_constraints;
...
Result<ast::Type, Error> semantic::get_function_type(
    Context& context,
    ast::Node* function_or_interface,
    std::vector<bool> call_args_mutability,
    std::vector<ast::Type> call_args,
    ast::Type call_type
) {
    // Get function called
    ast::FunctionNode* function = nullptr;

    if (std::holds_alternative<ast::FunctionNode>(*function_or_interface)) {
        function = &std::get<ast::FunctionNode>(*function_or_interface);
    } else if (std::holds_alternative<ast::InterfaceNode>(*function_or_interface
               )) {
