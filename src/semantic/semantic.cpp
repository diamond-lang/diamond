...
void add_type_parameters(
    semantic::Context& context, ast::FunctionNode& node, ast::Type type,
    std::unordered_map<ast::Type, ast::FieldTypes>& unified_fields_constraints,
    std::unordered_map<ast::Type, Set<ast::InterfaceType>>&
        unified_interface_constraints
) {
    if (type.is_final_type_variable()) {
        ast::TypeParameter parameter;
        parameter.type = type;

        if (unified_interface_constraints.find(type) !=
            unified_interface_constraints.end()) {
            parameter.interface = unified_interface_constraints[type];
        }
        if (unified_fields_constraints.find(type) !=
            unified_fields_constraints.end()) {
            for (auto field_constraint : unified_fields_constraints[type]) {
                parameter.type.as_final_type_variable()
                    .field_constraints.push_back(field_constraint);
            }
        }
        ...
        for (auto parameter : type.as_nominal_type().parameters) {
            if (!parameter.is_concrete()) {
                add_type_parameters(
                    context, node, parameter, unified_fields_constraints,
                    unified_interface_constraints
                );
            }
        }
...
Result<Ok, Errors> semantic::analyze_module(
...
Result<Ok, Error> semantic::analyze_block_or_expression(
    semantic::Context& context, ast::Node* node
) {
    ...
    // If we are in expression add constraint for expression with function
    // return type
    if (node->index() != ast::Block) {
        ...
        semantic::add_constraint(
            context, Set<ast::Type>(
                         {ast::get_type(node),
                          context.current_function.value()->return_type}
                     )
        );
    }

    // Merge type constraints that share elements
    context.type_inference.type_constraints =
        merge_sets_with_shared_elements<ast::Type>(
            context.type_inference.type_constraints
        );
    ...
    for (size_t i = 0; i < type_constraints.size(); i++) {
        ...
        while (j < type_constraints[i].size()) {
            if (!type_constraints[i].elements[j].is_nominal_type() ||
                type_constraints[i]
                        .elements[j]
                        .as_nominal_type()
                        .parameters.size() < 0) {
            ...
            for (size_t k = j + 1; k < type_constraints[i].size(); k++) {
                if (type_constraints[i].elements[j].is_nominal_type() &&
                    type_constraints[i].elements[k].is_nominal_type()) {
                    if (type_constraints[i]
                            .elements[j]
                            .as_nominal_type()
                            .name == type_constraints[i]
                                         .elements[k]
                                         .as_nominal_type()
                                         .name) {
                        assert(
    ...
    // If we are in a function check if return type is alone, if is alone it
    // means the function is void
    if (context.current_function.has_value() &&
        context.current_function.value()->return_type.is_type_variable()) {
        for (size_t i = 0; i < context.type_inference.type_constraints.size();
             i++) {
            if (context.type_inference.type_constraints[i].contains(
                    context.current_function.value()->return_type
                )) {
                if (context.type_inference.type_constraints[i].size() == 1) {
                    context.type_inference.type_constraints[i].insert(
                        ast::Type("None")
                    );
                }
            }
    ...
    for (size_t i = 0; i < context.type_inference.type_constraints.size();
        ...
        if (representatives.size() == 0) {
            context.type_inference.labeled_type_constraints
                [semantic::new_final_type_variable(context)] =
                context.type_inference.type_constraints[i];
        } else {
            ast::Type representative = representatives[0];
            for (size_t i = 1; i < representatives.size(); i++) {
                if (representative.is_nominal_type() &&
                    representatives[i].is_nominal_type()) {
                    if ((representative.as_nominal_type().name == "Boxed" ||
                         representatives[i].as_nominal_type().name == "Boxed"
                        ) &&
                        (representative.as_nominal_type().name == "Pointer" ||
                         representatives[i].as_nominal_type().name == "Pointer"
                        )) {
                        representative =
                            representative.as_nominal_type().name == "Boxed"
                                ? representative
                                : representatives[i];
                    } else if (representative.is_array() ||
                               representatives[i].is_array()) {
                        if (representative.array_size_known() &&
                            representatives[i].array_size_known()) {
                            assert(
                                representative.get_array_size() ==
                                representatives[i].get_array_size()
                            );
                    ...
                    } else if (representative.as_nominal_type().name !=
                               representatives[i].as_nominal_type().name) {
                        std::cout << representative.to_str() << " <> "
                                  << representatives[i].to_str() << "\n";
                        assert(false);
                    }
                } else {
                    std::cout << representative.to_str() << " <> "
            ...
            if (context.type_inference.labeled_type_constraints.find(
                    representative
                ) == context.type_inference.labeled_type_constraints.end()) {
                context.type_inference
                    .labeled_type_constraints[representative] =
                    context.type_inference.type_constraints[i];
            } else {
                context.type_inference.labeled_type_constraints[representative]
                    .merge(context.type_inference.type_constraints[i]);
            }
        }
    }
    ...
    for (auto type : context.type_inference.parameter_constraints) {
        ...
        for (auto parameter : type.second) {
            unified_parameter_constraints[unified_type].push_back(
                semantic::get_unified_type(context, parameter.type)
            );
            unified_type.as_final_type_variable()
                .parameter_constraints.push_back(
                    semantic::get_unified_type(context, parameter.type)
                );
        }

        semantic::set_unified_type(
            context, semantic::get_unified_type(context, type.first),
            unified_type
        );
    }
    ...
    for (auto type : context.type_inference.field_constraints) {
        ...
        for (auto field : type.second) {
            unified_field_constraints[unified_type][field.name] =
    ...
    for (auto it : context.type_inference.interface_constraints) {
        ...
        if (unified_interface_constraints.find(unified_type) ==
            unified_interface_constraints.end()) {
            unified_interface_constraints[unified_type] = it.second;
        } else {
            for (auto& interface : it.second.elements) {
                if (unified_interface_constraints[unified_type].contains(
                        ast::InterfaceType("number")
                    ) &&
                    interface.name == "float") {
                    unified_interface_constraints[unified_type].remove(
                        ast::InterfaceType("number")
                    );
                    unified_interface_constraints[unified_type].insert(
                        interface.name
                    );
                } else if (unified_interface_constraints[unified_type].contains(
                               ast::InterfaceType("float")
                           ) &&
                           interface.name == "number") {
                    // do nothing
                } else {
                    unified_interface_constraints[unified_type]
                        .insert(interface);
                }
            }
        }
    ...
    context.type_inference.interface_constraints =
        unified_interface_constraints;
    ...
    if (context.current_function.has_value() &&
        ...
        for (size_t i = 0; i < context.current_function.value()->args.size();
             i++) {
            context.current_function.value()->args[i]->type =
                semantic::get_unified_type(
                    context, context.current_function.value()->args[i]->type
                );
            if (!context.current_function.value()->args[i]->type.is_concrete(
                )) {
                add_type_parameters(
                    context, *context.current_function.value(),
                    context.current_function.value()->args[i]->type,
                    unified_field_constraints, unified_interface_constraints
                );
            }
        }
        ...
        context.current_function.value()->return_type =
            semantic::get_unified_type(
                context, context.current_function.value()->return_type
            );
        ...
        for (auto it : unified_field_constraints) {
            if (!context.current_function.value()->typed_parameter_aready_added(
                    it.first
                )) {
                add_type_parameters(
                    context, *context.current_function.value(), it.first,
                    unified_field_constraints, unified_interface_constraints
                );
            }
        }
...
Result<Ok, Error> semantic::analyze(
    semantic::Context& context, ast::BlockNode& node
) {
    return semantic::analyze_block_or_expression(context, (ast::Node*)&node);
}

Result<Ok, Error> semantic::analyze(
    semantic::Context& context, ast::FunctionNode& node
) {
    ...
    if (node.state == ast::FunctionBeingAnalyzed) {
        ...
        for (auto arg : node.args) {
            ...
            if (arg->type == ast::Type(ast::NoType{})) {
                ...
                semantic::add_constraint(
                    new_context, Set<ast::Type>({arg->type})
                );
            } else {
            ...
            // Create binding
            semantic::current_scope(new_context).variables_scope[identifier] =
                semantic::Binding((ast::Node*)arg);
        }
        ...
        if (node.return_type == ast::Type(ast::NoType{})) {
            ...
            semantic::add_constraint(
                new_context, Set<ast::Type>({node.return_type})
            );
        } else {
            // Analyze type if it has
        ...
        auto result =
            semantic::analyze_block_or_expression(new_context, node.body);
        if (result.is_error()) {
            context.errors.insert(
                context.errors.end(), new_context.errors.begin(),
                new_context.errors.end()
            );
            return Error{};
        }
...
Result<Ok, Error> semantic::analyze(
    semantic::Context& context, ast::Type& type
) {
    if (type.is_type_variable())
        return Ok{};
    else if (type.is_final_type_variable())
        return Ok{};
        ...
        return semantic::analyze(context, type.as_nominal_type().parameters[0]);
        ...
        std::optional<Binding> type_binding =
            semantic::get_binding(context, type.to_str());
        if (!type_binding.has_value()) {
            context.errors.push_back(
                Error{"Errors: Undefined type  \"" + type.to_str() + "\""}
            );
            return Error{};
        }
        ...
        type.as_nominal_type().type_definition =
            semantic::get_type_definition(type_binding.value());
        return Ok{};
    }
    assert(false);
    return Error{};
...
Result<Ok, Error> semantic::analyze(
    semantic::Context& context, ast::TypeNode& node
) {
...
// ------------------------------
bool semantic::are_types_compatible(
    ...
    if (function_type.is_concrete()) {
        return argument_type == function_type;
               ...
               function.get_type_parameter(function_type)
                       .value()
                       ->interface.size() > 0) {
        auto interfaces =
            function.get_type_parameter(function_type).value()->interface;
        for (auto interface_type : interfaces.elements) {
            if (!interface_type.is_compatible_with(
                    argument_type,
                    (ast::InterfaceNode*)function_and_types_scopes.get_binding(
                        interface_type.name
                    )
                )) {
                return false;
        ...
        if (function_type.as_nominal_type().name ==
                argument_type.as_nominal_type().name &&
            function_type.as_nominal_type().parameters.size() ==
                argument_type.as_nominal_type().parameters.size()) {
            for (size_t i = 0;
                 i < function_type.as_nominal_type().parameters.size(); i++) {
                if (!semantic::are_types_compatible(
                        function, function_and_types_scopes,
                        function_type.as_nominal_type().parameters[i],
                        argument_type.as_nominal_type().parameters[i]
                    )) {
                    return false;
                }
            }
        ...
        if (function_type.as_nominal_type().name == "Array" &&
            argument_type.is_array()) {
            for (size_t i = 0;
                 i < function_type.as_nominal_type().parameters.size(); i++) {
                if (!semantic::are_types_compatible(
                        function, function_and_types_scopes,
...
bool semantic::are_types_compatible(
    ast::FunctionNode& function,
    semantic::FunctionsAndTypesScopes& function_and_types_scopes,
    std::vector<ast::Type> function_types, std::vector<ast::Type> argument_types
) {
    for (size_t i = 0; i < function_types.size(); i++) {
        if (!semantic::are_types_compatible(
                function, function_and_types_scopes, function_types[i],
                argument_types[i]
            )) {
            return false;
        }
...
bool semantic::are_arguments_compatible(
    ast::FunctionNode& function,
    semantic::FunctionsAndTypesScopes& function_and_types_scopes,
    std::vector<bool> call_args_mutability,
    std::vector<ast::Type> function_types, std::vector<ast::Type> argument_types
) {
    ...
    if (!semantic::are_types_compatible(
            function, function_and_types_scopes,
            function_types[function_types.size() - 1],
            argument_types[argument_types.size() - 1]
        )) {
        return false;
    }
    return true;
...
Result<ast::Type, Error> get_field_type(
    std::string field, ast::Type struct_type
) {
    assert(
        struct_type.is_nominal_type() &&
        struct_type.as_nominal_type().type_definition
    );
    ...
    for (size_t i = 0;
        ...
        if (field ==
            ...
            return struct_type.as_nominal_type()
...
void add_argument_type_to_specialization(
    ast::FunctionSpecialization& specialization,
    std::vector<ast::TypeParameter>& type_parameters, ast::Type function_type,
    ...
    if (function_type.is_final_type_variable() &&
        ...
        if (specialization.type_bindings.find(
                function_type.as_final_type_variable().id
            ) == specialization.type_bindings.end()) {
            specialization
                .type_bindings[function_type.as_final_type_variable().id] =
                argument_type;
        }
        // Else compare with previous type founded for it
        else if (specialization
                     .type_bindings[function_type.as_final_type_variable()
                                        .id] != argument_type) {
            assert(false);
        }

        // If function argument has constraints
        if (ast::get_type_parameter(type_parameters, function_type)
                .has_value()) {
            ast::TypeParameter* type_parameter =
                ast::get_type_parameter(type_parameters, function_type).value();
            auto& field_constraints =
                type_parameter->type.as_final_type_variable().field_constraints;

            // If the parameter has field constraints
            for (size_t i = 0; i < field_constraints.size(); i++) {
                add_argument_type_to_specialization(
                    specialization, type_parameters, field_constraints[i].type,
                    get_field_type(field_constraints[i].name, argument_type)
                        .get_value()
                );
            }
        }
        ...
        auto parameter_constraints =
            function_type.as_final_type_variable().parameter_constraints;
        for (size_t i = 0; i < parameter_constraints.size(); i++) {
            add_argument_type_to_specialization(
                specialization, type_parameters, parameter_constraints[i],
                argument_type.as_nominal_type().parameters[i]
            );
        }
    } else if (function_type.is_concrete() && function_type != argument_type) {
        assert(false);
        ...
        for (size_t i = 0;
             i < function_type.as_nominal_type().parameters.size(); i++) {
            add_argument_type_to_specialization(
                specialization, type_parameters,
                function_type.as_nominal_type().parameters[i],
                argument_type.as_nominal_type().parameters[i]
            );
        }
        ...
        assert(
            function_type.as_nominal_type().name ==
            argument_type.as_nominal_type().name
        );
        assert(
            function_type.as_nominal_type().parameters.size() ==
...
Result<ast::Type, Error> semantic::get_function_type(
    Context& context, ast::Node* function_or_interface,
    std::vector<bool> call_args_mutability, std::vector<ast::Type> call_args,
    ast::Type call_type
) {
    // Get function called
    ...
    if (function_or_interface->index() == ast::Function) {
        ...
        for (auto it : interface.functions) {
            ...
            if (semantic::are_arguments_compatible(
                    *it, context.scopes.functions_and_types_scopes,
                    call_args_mutability, function_types, call_types
                )) {
        ...
        if (function == nullptr) {
            std::cout << "No implementation of " << interface.identifier->value
                      << " for call types \n";
            assert(false);
        }
    } else {
    ...
    if (function->state == ast::FunctionCompletelyTyped) {
        if (!function->is_used) {
            if (!function->is_builtin && !function->is_extern) {
                ...
                if (context.current_module == function->module_path) {
                    ...
                    auto result = semantic::add_scope(
                        new_context,
                        *(context.ast->modules[function->module_path.string()])
                    );
                    assert(result.is_ok());
                }
    ...
    // If is generic
        ...
        for (size_t i = 0; i < function->args.size(); i++) {
            ...
            add_argument_type_to_specialization(
                specialization, function->type_parameters, function_type,
                call_type
            );
            specialization.args.push_back(call_type);
        }
        add_argument_type_to_specialization(
            specialization, function->type_parameters, function->return_type,
            call_type
        );

        specialization.return_type = ast::try_to_get_concrete_type(
            function->return_type, specialization.type_bindings
        );
        if (specialization.return_type.is_final_type_variable()) {
            specialization.type_bindings
                [specialization.return_type.as_final_type_variable().id] =
                ast::get_default_type(function
                                          ->get_type_parameter(
        ...
        if (!function->is_builtin) {
            ...
            new_context.init_with(context.ast);
            new_context.current_function = function;
            new_context.type_inference.type_bindings =
                specialization.type_bindings;
            ...
            for (auto field :
                 struct_type.as_nominal_type().type_definition->fields) {
                auto print_function =
                    semantic::get_binding(context, "printWithoutLineEnding");
                (void)semantic::get_function_type(
                    context, print_function.value().value, {false},
                    {field->type}, ast::Type("None")
                );
            }
        }
