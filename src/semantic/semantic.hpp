...
namespace semantic {
	...
	struct Context {
		...
		Result<Ok, Error> get_type_of_generic_function(ast::CallNode& call, ast::FunctionNode* function);
		Result<Ok, Error> check_constraints(std::unordered_map<std::string, ast::Type>& type_bindings, ast::FunctionNode* function);
	};
}
